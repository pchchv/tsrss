...
export default function Sidebar({ visible, onClose }: SidebarProps) {
  ...
  return (
    <div>
      ...
      <div
        className={
          (visible ? "fixed sm:hidden " : "hidden ") +
          " z-30 inset-0 w-full min-h-screen bg-black bg-opacity-60"
        }
      />
      <div
        ref={ref}
        className={
          ...
          " transform sm:translate-x-0 z-40 transition duration-500 ease-in-out flex flex-col bg-sidebar dark:bg-darksidebar px-4 w-64 h-screen border-r dark:border-gray-900 fixed overflow-y-auto"
        }
      >
        <button className="sm:hidden flex justify-end pt-4" onClick={onClose}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="stroke-current text-gray-400 h-5 w-5"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
        ...
        <div className="flex tracking-wide items-center justify-between mb-4 p-1">
          ...
          <button
            onClick={() => {
              onClose();
              dispatch(showView({ viewName: "create_feed" }));
            }}
        ...
        {feeds &&
          feeds.map((feed, index) => (
            <Dropdown key={index} feed={feed} onClose={onClose} />
          ))}
      </div>
    </div>
  );
