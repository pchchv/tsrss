...
// How to handle create new feed.
// Swap between views.
export default function FeedProviderItem({
  feedProvider,
}: FeedProviderItemProps) {
  ...
  const subscribed = feeds.some((feed) =>
    feed.feedProviders.some(
      (provider) => provider.feedId === feedProvider.feedId,
    ),
  );
  ...
  const FallbackComponent = () => {
    return (
      <svg
        ...
        className="stroke-current h-8 w-8 text-accent p-px rounded bg-white shadow"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
      >
    ...
    <div className="flex justify-between mb-10 items-center gap-4">
      <div className="flex flex-row items-center gap-4 sm:gap-8">
        ...
        <div className="flex flex-col">
          <Link
            to={`/provider/${encodeURIComponent(feedProvider.feedId)}`}
            className="font-medium text-base sm:text-xl hover:underline primary-text"
          >
            {feedProvider.title}
          </Link>
          <a
            ...
            className="secondary-text hover:underline text-xs sm:text-base"
          >
            {getRootURL()}
          </a>
        </div>
      ...
      <div ref={container} className="relative">
        <button
          ...
          className={
            "w-24 sm:w-28 " + (subscribed ? "following-btn" : "follow-btn")
          }
        >
