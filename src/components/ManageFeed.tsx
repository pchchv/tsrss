        ...
        <>
          <div className="flex mb-8 gap-6 border-b text-gray-400">
            <button
              ...
              className={
                activeTab === "details"
                  ? "p-2 -m-px text-accent border-b-2 border-accent font-semibold"
                  : "p-2"
              }
            >
              Details
          ...
          {activeTab === "details" ? (
            <>
              <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col">
                ...
                <input
                  ...
                  className={
                    "bg-gray-100 dark:bg-gray-800 rounded-lg p-4 mb-6 w-full focus:outline-none primary-text " +
                    (errors.title && "border border-accent")
                  }
                  {...register("title", {
                    required: true,
                    validate: (value) =>
                      value.toLowerCase() === feed.title.toLowerCase() ||
                      !feeds.some(
                ...
                {errors.title && errors.title.type === "required" && (
                  <p className="text-accent mb-6">This field is required!</p>
                )}
                ...
                <div className="flex gap-4">
                  <button className="following-btn" type="submit">
                    SAVE
                  </button>
                  <button
            ...
            <div>
              {feed.feedProviders && feed.feedProviders.length > 0 ? (
                feed.feedProviders.map((provider, index) => (
                  <FeedProviderItem key={index} feedProvider={provider} />
                ))
              ) : (
                <p className="secondary-text text-2xl">
                  You are not following any providers!
                </p>
              )}
            </div>
          )}
