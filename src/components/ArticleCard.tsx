...
export default function ArticleCard({ article }: ArticleCardProps) {
  const FallbackItemImageComponent = () => {
    return (
      <div
        className="flex justify-center items-center bg-gray-100 dark:bg-darksidebar rounded-lg shadow"
        style={{ height: 100, minWidth: 200 }}
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          className="stroke-current h-16 w-16 text-accent"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
        >
    ...
    <div className="flex sm:flex-row flex-col mb-16 gap-4 sm:gap-8 items-start">
      {article.image && (
        <Image
          ...
          FallbackComponent={FallbackItemImageComponent}
          src={article.image}
          style={{ height: 100, minWidth: 200 }}
        />
      )}
      <div className="w-full">
        <div>
          <div className="flex gap-2 mb-1 items-center">
            <p className="text-sm secondary-text font-medium">
              {article.authors} / {article.published}
            </p>
          </div>
        </div>
        <a
          className="primary-text font-semibold sm:text-2xl hover:underline"
          href={article.link}
          target="_blank"
          rel="noreferrer"
        >
        ...
        {article.description && article.description !== "null" && (
          <p className="secondary-text text-sm sm:text-base">
