...
export default function Feed() {
  ...
  const feed = useAppSelector((state) =>
    state.feed.feed.find((f) => f.id === id),
  );
  ...
  const getFeed = useCallback(() => {
    if (hasProviders()) {
      let requests = feed!.feedProviders.map((provider) =>
        axios.get(provider.feedId.slice(5)).catch((e) => null),
      );
      axios
        .all(requests)
        .then(
          axios.spread((...responses) => {
            if (responses && responses !== null) {
              ...
              return parse2(
                xmlObjects,
                feed!.feedProviders.map((provider) => provider.title),
              );
            } else {
              throw new Error(
                "Failed to retrieve any response from providers.",
              );
            }
          }),
        )
  ...
  if (!feed) {
    return <p className="secondary-text text-2xl">No Feed!</p>;
  }

  return (
    <div className="flex flex-col">
      <div className="flex justify-between items-center mb-6">
        <h1 className="primary-text font-bold sm:text-7xl text-4xl">
          {feed.title}
        </h1>
        <div
          className={
            feed.feedProviders.length > 0 ? "flex items-center gap-6" : "hidden"
          }
        >
          ...
          <div ref={container} className="relative mt-2">
            <button onClick={() => setDropdownVisible(!dropdownVisible)}>
              <svg
                ...
                className="stroke-current text-gray-400 h-6 w-6"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
            ...
            {dropdownVisible && (
              <div className="dark:bg-darksidebar absolute right-0 flex flex-col gap-2 z-10 bg-white w-48 mt-1 p-4 shadow-lg rounded-xl">
                <button
                  ...
                  className="primary-text text-left dark:hover:bg-gray-800 hover:bg-gray-200 p-1 rounded"
                >
                  Manage Feed
                </button>
                <button
                  onClick={showDeleteFeed}
                  className="text-accent text-left dark:hover:bg-gray-800 hover:bg-gray-200 p-1 rounded"
                >
                  Delete Feed
                </button>
              </div>
      ...
      {!hasProviders() ? (
        <p className="secondary-text text-2xl">
          You haven"t subscribed to any feeds!
        </p>
      ) : error ? (
